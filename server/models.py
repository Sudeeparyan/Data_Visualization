...
class InputFiles(db.Model):
    ...
    parent_projects = db.relationship(
        'Projects', back_populates='child_input_file')
...
class Results(db.Model):
    ...
    trained_model_id = db.Column(db.Integer, db.ForeignKey(
        'trained_models.trained_model_id'), nullable=False)
    parent_trained_model = db.relationship(
        'TrainedModels', back_populates='child_results')

    input_file_id = db.Column(db.Integer, db.ForeignKey(
        'input_files.input_file_id'), nullable=False)
    parent_input_files = db.relationship(
        'InputFiles', back_populates='child_results')
