...
def generate_predictions(model_path: str, file_path: str, project_id: int) -> str:
    ...
    x_line = np.linspace(dependent_column.min(), dependent_column.max(
    ), independent_column.shape[0]).reshape(-1, 1)
    ...
    actual_coordinates = pd.DataFrame(
        {actual_data.columns[0]: dependent_column, actual_data.columns[1]: independent_column})
    error_coordinates = pd.DataFrame(
        {'error_X': x_line.flatten(), 'error_Y': error})
    ...
    result.to_csv(
        file_path+f"//{project_id}_{time_stamp}_result.csv", index=False)
