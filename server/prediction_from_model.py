...
def prediction_using_trained_model(model_path: str, file_path: str, project_id: int) -> str:
    ...
    with open(model_path, "rb") as f:
        ...
        result_csv = pd.concat([result_csv, pd.DataFrame({'error_X': X, 'error_Y': error}), actual_coordinates], axis=1)
        
        file_path = os.path.abspath(os.path.join(os.getcwd(),"storage","media_files","project_csv",project_id))
